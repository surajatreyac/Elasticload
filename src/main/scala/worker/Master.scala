...
class Master(workTimeout: FiniteDuration) extends PersistentActor with ActorLogging {
  ...
  override def receiveCommand: Receive = {
        ...
        workers.get(workerId) match {
          case Some(s @ WorkerState(_, Idle)) =>
            ...
            persist(WorkStarted(work.workId)) { event =>
              workState = workState.updated(event)
              log.info(s"Work state: ${workState.workInProgress}")
    ...
    case MasterWorkerProtocol.WorkIsDone(workerId, workId, result) =>
      ...
      }else if( workState.hasWork && (workState.pendingWork.last.workId == workId)){
        log.info("Ack came later but the work was removed from inprogress state to pending state")
        sender() ! MasterWorkerProtocol.Ack(workId)
      } else if (!workState.isInProgress(workId) ) {
        log.info(s"Pending work: ${workState.pendingWork}")
        log.info(s"workState : ${workState.workInProgress}")
